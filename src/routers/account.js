                        ...
                        account.save().then(account=>{
                            ...
                            try{
                                const transporter = nodemailer.createTransport({
                                    ...
                                    // secure: false,
                                    auth:{
                                        user:"donialuzzam@gmail.com",
                                        pass:"050617Islam"
                                    }
                                })
    
                                const mailOptions = {
                                    from:'donialuzzam@gmail.com',
                                    to:account.email,
                                    subject:"Verify Account",
                                    text:"account",
                                    // html:"<h1>hello</h1>"
                                    html: 'Hello,\n\n' + 'Please verify your account by clicking the link: \nhttp:\/\/' + req.headers.host + '\/verification\/' + account._id + '\/' + token + '.\n'
                                }
                                console.log(token, "token")
                                transporter.sendMail(mailOptions,(error,info)=>{
                                    console.log(mailOptions,'masuk')
